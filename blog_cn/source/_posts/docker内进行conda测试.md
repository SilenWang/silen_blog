...
- 初始化一个centos 7的容器, 然后安装`miniconda`, 因为容器是新初始化的所以啥都没...要安装一下必要的东西
...
- 本次要模拟的是普通用户使用conda安装sscClust(root获取不可), 因此所有需要的包均不使用yum安装, R包则进入R后进行安装, 不使用conda提供的二进制版本(防止有包conda未收录, 依赖出问题).
