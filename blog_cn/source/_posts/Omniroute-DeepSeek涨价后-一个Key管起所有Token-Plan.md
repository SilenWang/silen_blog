...
前两个坑好歹能忍，这个坑是真能气死人：**在默认配置下，Omniroute 并不允许重型任务并发，跑着跑着 Agent 会突然断掉。**

现在的 Agent（Claude Code、Codex、opencode 这些）干活从来不是一个请求一个请求来的。一个大任务会同时拆出好几个子任务，并行地打一堆 LLM 请求——比如 opencode 的子 Agent、Claude Code 的并行工具调用。这一下并发就上去了。

而 Omniroute 这边，**每个账号（连接）默认都有并发上限**，超出的请求只能排队等着：账号信号量默认最多排 20 个、等 30 秒，再往上就直接拒绝；请求队列那边还有执行上限。重型任务一开、并发一超，后面的请求要么被压住排队，要么直接被拒。客户端 Agent 收到这种错误（429、超时、队列满），很多就直接判定为致命错误，整个会话啪地一下断掉——你看到的就是"用着用着突然断了"。

更坑的是，这个限制是**按账号（连接）生效的，不是全局的**。你以为填了并发数就是全局上限？不，它只约束那一个账号，别的账号、别的商家各有各的额度。想做真正的全局并发上限，目前只能等（[#7778](https://github.com/diegosouzapw/OmniRoute/issues/7778)）。

我的应对办法：

1. 给每个连接设置 `max_concurrent`，按上游账号真实的并发能力来填（比如 GLM 这类只吃得下一两个并发的，就填 1~2），别让它们默认裸奔，也别贪高。
2. 真要并行跑重型任务时，要么给 Agent 本身限一下并发，要么去 `Settings → Resilience → Request Queue` 把队列参数调大。
3. 排查时留意是不是 session affinity 把会话钉在了已饱和的账号上，白白干等那 30 秒。
