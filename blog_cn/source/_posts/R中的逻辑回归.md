...
## 简介
逻辑(logistic)回归是广义线性模型的一种, 其特点在于因变量是分类变量, 通过将所有分类中的一个设为对照, 将其他的分类选项发生的的可能都与对照做比较的方式, 将分类变量转变成连续变量, 然后进行模型构建. 在逻辑回归中, 变量的系数取`exp(x)`后即为这个变量对应的OR值, 实际意义为, 该变量每增加一个单位, 结果被分类为
1的可能性将提高`exp(x)`.
...
```r
test_data$prob <- predict(model.step, test_data, type = "response") # response的话结果是0~1的概率, 不指定默认是个常数
