...
- `COPY`: 从外部(宿主机)复制文件到镜像内, 第一个参数是外部路径, 第二个参数是内部路径, 具体参数用法类似`cp`, 似乎不能多个文件同时使用, 没有尝试过通配符是否有效
...
- `COPY`这个关键字在复制外部路径时, 只能获取`docker build`制定的构建路径下以及其下层的文件. 这是受docker运行时**上下文(context)**的限制. 
