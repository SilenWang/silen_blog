...
子代理的重要初衷之一是隔离上下文，即使在模型上下文长度已经比过去扩展了数倍的今天，一次对话的上下文也不足以将一项工作的所有代码和执行内容存下来，因此，将有关联但是具体内容并不直接相关的内容，拆分到subagent的对话中，在agent的对话中只保留subagent的工作摘要和工作结果，有利于让agent在执行过程中保持目标，不要跑偏。
...
可是唯独保存信息来源链接这个看上去很简单的事情，初开发时用的Minimax-M2.5就是做不到，保存下来的链接10个9个都是错的。即使将Temperature设置到最低，附加更多的提示词，也无法保存真实的链接。

核对信息的时候，情况更是让人哭笑不得，它保存链接的时候，极大概率会修改链接中单词的单复数，和一些代号字符串。比如:
...
对于Subagent，一旦离开了之前的上下文，或没有明确指定实现方式，它每一次的实现方式，可能会完全不相同。比如这一次写js，下一次写python，再下一次直接Shell执行。整个过程像极了早期文生图模型的反复抽卡...
